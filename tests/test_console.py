#!/usr/bin/python3
"""
    This module contains unit tests for the console (command interpreter).
"""
import os
import unittest
...
class TestHBNBCommand(unittest.TestCase):
    """
    Represents a test suite for the HBNBCommand class.
    """

    @unittest.skipIf(
        os.getenv('HBNB_TYPE_STORAGE') == 'db', 'Skipping FileStorage test')
    def test_fs_create(self):
        """
        Tests the 'create' command with FileStorage.
        """
        with patch('sys.stdout', new=StringIO()) as cout:
            ...
            console.onecmd('show City {}'.format(mdl_id))
            ...
            clear_stream(cout)
            console.onecmd('show User {}'.format(mdl_id))
    ...
    @unittest.skipIf(
        os.getenv('HBNB_TYPE_STORAGE') != 'db', 'Skipping DBStorage test')
    def test_db_create(self):
        """
        Tests the 'create' command with Database Storage.
        """
        with patch('sys.stdout', new=StringIO()) as cout:
            console = HBNBCommand()
    ...
    def test_db_show(self):
        ...
        with patch('sys.stdout', new=StringIO()) as cout:
            console = HBNBCommand()
            # Attempting to show a User instance
